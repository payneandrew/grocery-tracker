...
import type { ReceiptData, SpendingData, CategoryData } from "./types"
...
export async function getRecentReceipts(): Promise<ReceiptData[]> {
  ...
  return receipts.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 10)
}
...
export async function getSpendingData(timeframe: "week" | "month" | "year"): Promise<SpendingData> {
  const receipts = await getAllReceipts()
